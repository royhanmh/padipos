  ...
  return (
    <>
      <button
        ...
        className="relative flex h-11 items-center gap-2 rounded-[10px] px-3 text-base text-[#6B6B6B] transition hover:bg-[#F5F5F5] hover:text-[#3572EF] max-lg:h-10 max-lg:w-10 max-lg:justify-center max-lg:px-0"
      >
      ...
      {isArchiveOpen ? (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            className="flex max-h-[90vh] w-full max-w-[760px] flex-col rounded-[10px] bg-white shadow-[0_24px_64px_rgba(17,24,39,0.2)] md:rounded-[10px]"
          ...
          >
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden rounded-b-[10px] bg-[#F8F9FB] px-4 py-5 md:gap-5 md:rounded-b-[10px] md:px-7 md:py-7">
              <div className="z-10 flex shrink-0 flex-col gap-3 md:flex-row md:items-center">
                ...
                <div className="flex gap-3 max-md:grid max-md:grid-cols-[1fr_auto]">
                  ...
                  <button 
                    ...
                    className="h-[46px] rounded-[10px] bg-[#3572EF] px-7 text-[15px] font-medium text-white shadow-[0_4px_12px_rgba(53,114,239,0.15)] transition hover:brightness-105 active:scale-95"
                  >
                    Search
                  </button>
                </div>
              ...
              <div className="flex-1 space-y-[14px] overflow-y-auto pr-1 scrollbar-hide">
                {filteredOrders.length === 0 ? (
                  <div className="rounded-[10px] border border-dashed border-[#D4DCEB] bg-white px-6 py-12 text-center">
                    <p className="text-[18px] font-semibold text-[#6E6E6E] md:text-[20px]">No archive orders found</p>
                  </div>
                ) : (
                  filteredOrders.map((order) => (
                    <div key={order.id} className="group relative flex flex-col justify-between rounded-[10px] bg-white p-4 shadow-[0_4px_16px_rgba(0,0,0,0.03)] transition hover:shadow-[0_4px_24px_rgba(55,114,239,0.08)] md:p-[22px]">
