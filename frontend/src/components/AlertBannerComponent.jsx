...
const AlertBannerComponent = ({
  ...
  const authBodyResponsiveClass = isAuthLayout
    ? "items-center max-lg:gap-4 max-lg:px-5 max-lg:py-4"
    : "";
  const authIconResponsiveClass = isAuthLayout ? "mt-0 max-lg:text-[24px]" : "";
  const authMessageResponsiveClass = isAuthLayout
    ? "mt-0 self-center text-[18px] leading-7 max-lg:text-[16px] max-lg:leading-6 break-words"
    : "";
  ...
  const dismissButtonResponsiveClass = isAuthLayout
    ? "mt-0 self-center max-lg:-mr-1 max-lg:flex max-lg:h-8 max-lg:w-8 max-lg:items-center max-lg:justify-center"
    : "";

  return (
    <div className={`${styles.wrapper} ${className}`.trim()}>
      <div
        className={`flex items-start gap-4 ${styles.body} ${authBodyResponsiveClass}`.trim()}
      >
        {variant === "success" ? (
          <PiCheckCircleLight
            className={`${styles.icon} ${authIconResponsiveClass}`.trim()}
          />
        ) : null}
        <p className={`${styles.message} ${authMessageResponsiveClass}`.trim()}>
          {message}
        </p>
        {dismissible ? (
          <button
          ...
          >
            <PiXLight
              className={isAuthLayout ? "text-[24px]" : "text-[20px]"}
            />
          </button>
        ) : null}
      </div>
